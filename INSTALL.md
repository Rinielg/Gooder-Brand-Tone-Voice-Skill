...
| Skill | What it does |
|-------|-------------|
| **gooder-brand-tone-voice** | Train a brand voice profile from your brand documents (PDF, DOCX, TXT, PNG, JPG, HTML, URLs) |
...
You should see `gooder-brand-tone-voice`, `gooder-tone-validator`, and `gooder-ux-guide` listed.
...
**Requires:** A brand profile trained with gooder-brand-tone-voice.
...
1. **Train** — Run gooder-brand-tone-voice with your brand materials. Takes 10–20 minutes the first time.
...
Run gooder-brand-tone-voice again with the same brand name. New materials are merged into the existing profile — nothing is overwritten unless you explicitly ask to update a field.
...
- Run gooder-brand-tone-voice first to create the profile
- Check the brand name matches exactly (case-sensitive, hyphens not spaces)

**Profile saved to wrong location:**
- The first time you run gooder-brand-tone-voice, it asks you to confirm the storage path
