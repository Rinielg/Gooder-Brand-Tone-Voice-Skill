...
description: Use when validating or QA-ing any copy against a brand's tone and voice. Scores content across 8 weighted dimensions and returns flags, scores, and improvement suggestions. Works with brand profiles created by gooder-brand-tone-voice. Supports email, SMS, push notifications, UX copy, social, concept copy, and general copy. Trigger this skill whenever the user asks to "check", "review", "score", "validate", "QA", or "audit" any written content against a brand — even if they don't use the word "tone" or "voice". Also trigger when the user pastes copy and asks "does this sound on-brand?", "is this right?", "how does this read?", or any variation of requesting feedback on written content quality relative to a brand. If a gooder brand profile exists and the user shares copy for review, use this skill — don't wait for them to mention validation explicitly.
---
...
2. Attempt to Read the profile from `~/.claude/gooder/profiles/<brand-name>.md` (or the path provided).
   - If not found: "I couldn't find a profile for [brand name]. Has it been trained yet? Use the **gooder-brand-tone-voice** skill to create one first."
